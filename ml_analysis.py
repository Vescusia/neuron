...
@click.argument("model-path", type=click.Path(dir_okay=True, file_okay=False))
...
def use_comp_model(model_path, report_index, champions, patch, tier, division):
    ...
    if tier in lib.TIERS[-3:]:
        click.echo("Master+, assuming division None")
        division = None
    ...
    print(model)
    click.echo(f"The Comp has a probable winrate of {model(game):.2%}")
